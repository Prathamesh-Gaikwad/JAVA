...
public class Registration implements ActionListener
{
	...
	static StringBuilder choice1Str;
	...
	Registration()
	{
		...
		head.setBounds(1200, 100, 800, 50);
		head.setFont(font);
		
		loginType.setBounds(1200, 200, 800, 30);
		loginType.setFont(textF);
		
		choice1.setBounds(1200, 240, 200, 30);
		choice1.setFont(butF);
		
		choice1.add("Employee");
		choice1.add("Project Manager");
		...
		frame.setTitle("[-TaskTracker LOGIN-]");
	...
	void myRegistration()
	{
		...
		regF = new JFrame("[-TaskTracker REGISTRATION-]");
		...
		JLabel registrationType = new JLabel("REGISTRATION TYPE : ");
		...
		choice2.add("Employee");
		choice2.add("Project Manager");
		
		headL.setBounds(1200, 70, 400, 50);
		headL.setFont(font);
		
		registrationType.setBounds(1100, 150, 400, 30);
		registrationType.setFont(textF);
		
		choice2.setBounds(1100, 190, 200, 30);
		choice2.setFont(butF);
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		{	
			...
			if(login == event.getSource())
			{
				...
				choice1Str = new StringBuilder();
				choice1Str.replace(0, choice1Str.length(), choice1.getItem(choice1.getSelectedIndex()));
				
				if(choice1Str.toString().equals("Employee"))
					sql2 = "select Password from Users where UserName = ?";
				else
					sql2 = "select Password from manager where UserName = ?;";
				...
				if(result.next())
				{
					...
					if(Arrays.equals(original, passw))
					{
						...
						JOptionPane.showMessageDialog(frame, message, "[-TaskTracker (login)-]", messageType);
			...
			if(event.getSource() == regB)
			{
				...
				boolean flag = false;
				StringBuilder str;
				StringBuilder choiceStr;
				StringBuilder sqlStr;
				
				choiceStr = new StringBuilder();
				sqlStr = new StringBuilder();
				
				choiceStr.replace(0, choiceStr.length(), choice2.getItem(choice2.getSelectedIndex()));
				...
				if(flag)
				{
					...
					if(choiceStr.toString().equals("Employee"))
						sqlStr.replace(0, sqlStr.length(), "INSERT INTO Users (UserName, Password, Gmail, reminder) VALUES (?, ?, ?, ?);");
					else
						sqlStr.replace(0, sqlStr.length(), "insert into manager (username, password, gmail, reminder) values (?, ?, ?, ?);");
					
					statement = conn.prepareStatement(sqlStr.toString());
					...
					{	
						...
						messageType = JOptionPane.INFORMATION_MESSAGE;
						JOptionPane.showMessageDialog(frame, str.toString(), "[-TaskTracker (registration)-]", messageType);
						//registration done..
	...
	boolean ValidCredintials(String userName, String mail, char array[], int hour, int min, String meridiem)
	{
		...
		if(name)
		{
			...
			if(result)
			{
				...
				if(upper)
				{
					if(smaller)
					{
						...
						{
							messageType = JOptionPane.ERROR_MESSAGE;
							JOptionPane.showMessageDialog(regF, "PASSWORD MUST CONTAIN ONE DIGIT", "[-TaskTracker (registration)-]", messageType);
							return false;														
						}
					}
					else
					{
						messageType = JOptionPane.ERROR_MESSAGE;
						JOptionPane.showMessageDialog(regF, "PASSWORD MUST CONTAIN ONE SMALLCASE LETTER", "[-TaskTracker (registration)-]", messageType);
						return false;								
					}				
				}
				else
				{
					messageType = JOptionPane.ERROR_MESSAGE;
					JOptionPane.showMessageDialog(regF, "PASSWORD MUST CONTAIN ONE UPPERCASE LETTER", "[-TaskTracker (registration)-]", messageType);
					return false;							
				}
			}
			else
			{
				messageType = JOptionPane.ERROR_MESSAGE;
				JOptionPane.showMessageDialog(regF, "ENTER VALID MAIL ADDRESS", "[-TaskTracker (registration)-]", messageType);
				return false;			
			}		
		}
		else
		{
			messageType = JOptionPane.ERROR_MESSAGE;
			JOptionPane.showMessageDialog(regF, "ENTER VALID USERNAME", "[-TaskTracker (registration)-]", messageType);
			return false;		
		}
	}
...
class Home implements ActionListener
{
	...
	//select manager
	static Choice forEmployee;
	static JLabel labelName;
	JLabel labelManager;
	JButton selectManager;
	
	Home()
	{
		frame = new JFrame("[-TaskTracker-]");
		...
		help = new JMenu("ASSIGNED");
		userM = new JMenuItem("ASSIGNED TASK");
		...
		if(Registration.choice1.getItem(Registration.choice1.getSelectedIndex()).equals("Employee"))
	...
	void Body()
	{
		...
		labelManager = new JLabel("SELECT PROJECT MANAGER : ");
		labelName = new JLabel();
		forEmployee = new Choice();
		selectManager = new JButton("SAVE");

		forEmployee.setBounds(190, 10, 150, 25);
		labelManager.setBounds(10, 10, 180, 25);
		labelName.setBounds(10, 45, 200, 25);
		selectManager.setBounds(380, 10, 100, 25);

		//labelManager.setFont(textF1);
		//labelName.setFont(textF1);

		forEmployee.setBackground(Color.WHITE);
		forEmployee.setFont(newFont);
		
		//selectManager.setFont(textF);
		selectManager.setBackground(Color.GREEN);
		selectManager.addActionListener(this);

		fillChoice(forEmployee);
		...
		if(Registration.choice1.getItem(Registration.choice1.getSelectedIndex()).equals("Employee"))
		{
			...
			panel.add(forEmployee);
			panel.add(selectManager);
			panel.add(labelName);
		}
	...
	public void setProjectManagerName()
	{
		...
		try
		{
			try
			{
				...
				String nameU = rObj.getUserName();
							
				int userID = dObj.getUserID(nameU, conn);
				
				result = dObj.getManagerID(userID, conn);
				
				if(result.next())								
					result = dObj.getManagerName(result.getInt("managerid"), conn);	
				
				if(result.next())
					labelName.setText("PROJECT MANAGER : " + result.getString("username"));
			}
			catch(Exception e)
			{
				e.printStackTrace();
	...
	public void fillChoice(Choice choice)
	{
		...
		try
		{
			try
			{
				...
				result = dObj.getManagerName(conn);	
				
				while(result.next())
					choice.add(result.getString("username"));
			}
			catch(Exception e)
			{
				e.printStackTrace();
	...
	public void fillChoice(Choice choice, boolean bool)
	{
		StringBuilder nameU;
		Connection conn = null;
		ResultSet result = null;
		...
		try
		{
			try
			{
				...
				nameU.replace(0, nameU.length(), rObj.getUserName());
				
				int managerID = dObj.getUserID(nameU.toString(), conn, true);
								
				result = dObj.getEmployeeID(managerID, conn);
				
				while(result.next())
				{
					result1 = dObj.getUserName(result.getInt("userid"), conn);
					if(result1.next())
					{
						nameU.replace(0, nameU.length(), result1.getString("username"));
						choice.add(nameU.toString());
					}
				}			
			}
			catch(Exception e)
	...
	void newTask()
	{
		...
		frameNT = new JFrame("TaskTracker [ADD NEW TASK]");
		...
		if(Registration.choice1Str.toString().equals("Project Manager"))
			fillChoice(assigned, true);
		
		assignedTo.setBounds(1070, 720, 180, 30);
		assignedTo.setFont(textAdd1);
		assigned.setBounds(1270, 720, 150, 30);
		assigned.setBackground(Color.WHITE);
		...
		save.setBounds(1490, 720, 170, 30);
		...
		if(Registration.choice1Str.toString().equals("Project Manager"))
		{
			panelI2.add(assignedTo);
			panelI2.add(assigned);
		}
		...
		frameNT.setSize(screenSize.width-33,screenSize.height-33);
	}
	...
	void viewAll()
	{
		...
		JFrame frame = new JFrame("TaskTracker [VIEW ALL TASKS]");
		...
		try
		{
			...
			if(Registration.choice1Str.toString().equals("Employee"))
			{
				...
				result = dObj.getRecords(userID, conn, false);	
			}
			else
			{	
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getRecords(userID, conn, true);				
			}
	...
	void compTask()
	{
		...
		JFrame frame = new JFrame("TaskTracker [VIEW COMPLETED TASKS]");
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);			
				result = dObj.getCompRecords(userID, conn, false);	
			}
			else
			{	
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getCompRecords(userID, conn, true);	
			}			
	...
	void pendTask()
	{
		...
		JFrame frame = new JFrame("TaskTracker [VIEW PENDING TASKS]");
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);			
				result = dObj.getPendRecords(userID, conn, false);	
			}
			else
			{	
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getPendRecords(userID, conn, true);	
			}			
	...
	void assignTask()
	{
		...
		String colName[] = {"TASK NO", "TITLE", "DESCRIPTION", "DUE DATE", "STATUS", "ASSIGN TO"};
		String colName1[] = {"TASK NO", "TITLE", "DESCRIPTION", "DUE DATE", "STATUS"};
			
		JFrame frame = new JFrame("TaskTracker [VIEW ASSIGN TASKS]");
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);			
				result = dObj.getAssignRecords(userID, conn, false);	
			}
			else
			{	
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getAssignRecords(userID, conn, true);	
			}			
			...
			while(result.next())
			{
				...
				result1 = dObj.getUserName(userID, conn);	
				
				if(result1.next())
					userName1.replace(0, userName1.length(), result1.getString(1));
				
				if(Registration.choice1Str.toString().equals("Employee"))
				{
					Object obj[] = {taskID, userName, descrip, date, status};
					model.addRow(obj);	
				}
				else
				{	
					Object obj[] = {taskID, userName, descrip, date, status, userName1.toString()};	
					model.addRow(obj);
				}
			}
		}
	...
	void openFile(String fileName)
	{
		...
		frame = new JFrame("TaskTracker [OPEN FILE]");
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		{	
			...
			try
			{
				...
				if(Registration.choice1Str.toString().equals("Employee"))
				{
					...
					result = dObj.getTaskCount(no, conn, false);
				}
				else
				{	
					no = dObj.getUserID(userName, conn, true);				
					result = dObj.getTaskCount(no, conn, true);				
				}
		...
		else if(event.getSource() == save)
		{
			...
			else
			{
				...
				try
				{
					...
					if(Registration.choice1Str.toString().equals("Project Manager"))
					{
						if(assigned.getItem(assigned.getSelectedIndex()).equals("self"))
							sql1 = "select UserID from manager where UserName = ?;";
						else
						{
							sql2 = "select UserID from manager where UserName = ?;";
							statement = conn.prepareStatement(sql2);
							...
							if(result.next())
							{
								useridM = result.getInt(1);			//receives UserID
							}
							sql1 = "select UserID from Users where UserName = ?;";
							userName = assigned.getItem(assigned.getSelectedIndex());	
						}					
					}
					else
						sql1 = "select UserID from Users where UserName = ?;";
					...
					{
						if(assigned.getItem(assigned.getSelectedIndex()).equals("self"))
							sql1 = "insert into Tasks (Title, Description, DueDate, Status, managerid, owner) values (?, ?, ?, ?, ?, ?);";
						else
							sql1 = "insert into Tasks (Title, Description, DueDate, Status, userid, managerid, owner) values (?, ?, ?, ?, ?, ?, ?);";
						
					}
					else
						sql1 = "insert into Tasks (Title, Description, DueDate, Status, UserID, owner) values (?, ?, ?, ?, ?, ?);";
		...
		else if(event.getSource() == selectManager)
		{
			boolean bool = true;
			Connection conn = null;
			ResultSet result = null;
			StringBuilder nameManager;
						
			nameManager = new StringBuilder();
			nameManager.replace(0, nameManager.length(), forEmployee.getItem(forEmployee.getSelectedIndex()));
			...
			try
			{
				try
				{
					...
					int employeeID = dObj.getUserID(nameU, conn);
					
					result = dObj.getManagerID(nameManager.toString(), conn);
					
					if(result.next())				
						bool = dObj.setManagerName(employeeID, result.getInt("userid"), conn);	

					if(!bool)
					{
						JOptionPane.showMessageDialog(frame, "SELECTED SUCCESSFULLY", "[-SELECT PROJECT MANAGER-]", JOptionPane.INFORMATION_MESSAGE);	
						setProjectManagerName();
					}
					else
						JOptionPane.showMessageDialog(frame, "PROBLEM IN SELECTION TRY AGAIN ?", "[-SELECT PROJECT MANAGER-]", JOptionPane.ERROR_MESSAGE);					
				}
				catch(Exception e)
				{
					e.printStackTrace();
		...
		else if(event.getSource() == userM)
		{
			...
			try
			{
				...
				{
					no = dObj.getUserID(userName, conn);
					result = dObj.getAssignTaskCount(no, conn, false);
				}
				else
				{	
					no = dObj.getUserID(userName, conn, true);				
					result = dObj.getAssignTaskCount(no, conn, true);				
				}
				...
				if(0 < no)
					...
					JOptionPane.showMessageDialog(frame, "NO TASKS YET", "[-UNABLE TO VIEW ASSIGN TASKS-]", JOptionPane.ERROR_MESSAGE);							
			}
			catch(Exception e)
			{}		
		}
	...
	void setTaskSum()
	{
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);
				result = dObj.getTaskCount(userID, conn, false);
			}
			else
			{	
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getTaskCount(userID, conn, true);				
			}
			...
			//completed tasks...
			if(Registration.choice1Str.toString().equals("Employee"))
				result = dObj.getCompTaskCount(userID, conn, false);
			else
				result = dObj.getCompTaskCount(userID, conn, true);
			...
			//pending tasks...
			if(Registration.choice1Str.toString().equals("Employee"))
				result = dObj.getPendTaskCount(userID, conn, false);
			else
				result = dObj.getPendTaskCount(userID, conn, true);
			...
			//overdue tasks..		
			if(Registration.choice1Str.toString().equals("Employee"))
				result = dObj.getDueTaskCount(userID, conn, false);
			else
				result = dObj.getDueTaskCount(userID, conn, true);
	...
	void setNotify()
	{
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);
				result = dObj.getDueTasks(userID, conn, false);
			}
			else
			{
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getDueTasks(userID, conn, true);
			}
	...
	void setRecentTasks()
	{
		...
		try
		{
			...
			{
				userID = dObj.getUserID(userName, conn);
				result = dObj.getRecentTasks(userID, conn, false);
			}
			else
			{
				userID = dObj.getUserID(userName, conn, true);				
				result = dObj.getRecentTasks(userID, conn, true);
			}
...
class EditTask implements ActionListener
{
	...
	void edit()
	{
		...
		frame = new JFrame("TaskTracker EDIT TASK");
		textF1 = new Font("Arial", Font.BOLD, 17);
			...
			try
			{
				...
				else
				{
					try
					{
						...
						{
							userID = dObj.getUserID(userName, conn);
							result = dObj.getIDRecords(number, userID, conn, false);
						}
						else
						{	
							userID = dObj.getUserID(userName, conn, true);				
							result = dObj.getIDRecords(number, userID, conn, true);				
						}
		...
		else if(event.getSource() == save)
		{
			...
			else
			{
				...
				try
				{
					...
					{
						userID = dObj.getUserID(userName, conn);
						taskID = dObj.getTaskID(title, userID, conn, false);
						rowInserted  = dObj.setRecords(title, descrip, sqlDate, EditTask.status, taskID, userID, conn, false);
					}
					else
					{	
						userID = dObj.getUserID(userName, conn, true);				
						taskID = dObj.getTaskID(title, userID, conn, true);
						rowInserted  = dObj.setRecords(title, descrip, sqlDate, EditTask.status, taskID, userID, conn, true);
					}
			...
			{			
				try
				{
					...
					if(Registration.choice1Str.toString().equals("Employee"))
					{
						userID = dObj.getUserID(userName, conn);
						taskID = dObj.getTaskID(title, userID, conn, false);
						rowsDeleted = dObj.removeRecord(taskID, userID, conn, false);
					}
					else
					{	
						userID = dObj.getUserID(userName, conn, true);				
						taskID = dObj.getTaskID(title, userID, conn, true);
						rowsDeleted = dObj.removeRecord(taskID, userID, conn, true);
					}
