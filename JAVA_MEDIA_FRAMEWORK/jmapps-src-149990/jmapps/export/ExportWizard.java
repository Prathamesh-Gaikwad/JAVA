/*
 * @(#)ExportWizard.java	1.10 00/05/12
 *
...
//            MessageDialog.createErrorDialog ( frameOwner, JMFI18N.getResource("jmstudio.error.processor.realize") + " " + strFailMessage );
    ...
    private boolean doSaveFile () {
        ...
        try {
            ...
            dataSinkSave = Manager.createDataSink ( dsOutput, mediaDest );
        }
		...
		if (nAudioIndex >= arrAudioEnabled.length)
            ...
            try {
                if ( strAddress.equals("...")  ||  strPort.equals("") )
                    continue;
                managerSession = JMFUtils.createSessionManager ( strAddress, strPort, strTtl );
    ...
    private boolean doPreview () {
        ...
        // Start the processor
        try {
            player = Manager.createPlayer ( dsOutput );
            if ( player != null ) {
