...
public class CDMPanel extends JMPanel implements VectorEditor, ActionListener {
	...
	Vector currentList = deviceVector;
	deviceVector = getList(0);
	for (int i = 0; i < deviceVector.size(); i++) {
		...
		if (cdi != null) {
		    ...
		    //System.err.println("Removed device " + name);
		}
	    }
	}
        try {
	...
	if (autoVideo == null) {
        ...
        if ( autoAudio == null  &&  autoVideo == null  &&  autoVideoPlus == null )
            return;
